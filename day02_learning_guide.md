...
│ • Bắt tay phần    │   (Write1ToClear) │ • Mổ xẻ 5 Bug phần cứng    │   (Elevator Pitch)         │
│   cứng (Mermaid)  │                   │                            │                            │
└───────────────────┴───────────────────┴────────────────────────────┴────────────────────────────┘
```
...
### Sơ đồ Bus Matrix & Luồng Dữ liệu Phần cứng (Dataflow Architecture)
```text
       [ Tín hiệu UART từ bên ngoài (RX Pin: PB7) ]
          ...
          │  - Stream 2 (Circular Mode)      │      (dma_rx_ring_buffer)
          └──────────────────────────────────┘
                           │ (Đường truyền rảnh 1 frame time)
                           ▼
          ┌──────────────────────────────────┐
          │  Ngắt USART1 IDLE Line           │ ───► CPU chỉ thức giấc 1 LẦN DUY NHẤT!
          │  - Đánh thức CPU xử lý gói tin   │      (Invalidate D-Cache & đọc buffer)
          └──────────────────────────────────┘
```
...
### 📋 Sơ Đồ 1: Quy Trình Cấu Hình Tuần Tự Khởi Tạo UART1 & DMA2 RX Ring Buffer (Peripheral Configuration Pipeline)

Sơ đồ thể hiện toàn bộ các bước cấu hình tuần tự các thanh ghi để đưa UART1 và DMA2 vào trạng thái sẵn sàng nhận dữ liệu:

```mermaid
sequenceDiagram
    autonumber
    actor App as Application Code (uart.c)
    participant RCC as RCC (Clock Controller)
    participant GPIO as GPIOA & GPIOB (Pins AF7)
    participant DMA as DMA2 Stream 2 (Circular RX)
    participant USART as USART1 Peripheral (APB2)
    participant NVIC as Cortex-M7 NVIC

    Note over App,NVIC: BƯỚC 1: CẤP CLOCK CHO NGOẠI VI (AHB1 & APB2 BUS)
    App->>RCC: 1. Cấp xung GPIOA, GPIOB, DMA2: RCC->AHB1ENR |= (GPIOAEN | GPIOBEN | DMA2EN)
    App->>RCC: 2. Cấp xung USART1: RCC->APB2ENR |= RCC_APB2ENR_USART1EN

    Note over App,GPIO: BƯỚC 2: CẤU HÌNH GHÉP KÊNH CHÂN GPIO (PIN MULTIPLEXING AF7)
    App->>GPIO: 3. Chân PA9 (TX): MODER=10b (AF), AFRH9=0111b (AF7 USART1_TX), OSPEEDR=High
    App->>GPIO: 4. Chân PB7 (RX): MODER=10b (AF), AFRL7=0111b (AF7 USART1_RX), PUPDR=01b (Pull-up)

    Note over App,DMA: BƯỚC 3: CẤU HÌNH BỘ TRUY CẬP BỘ NHỚ TRỰC TIẾP (DMA2 STREAM 2)
    App->>DMA: 5. Reset Stream: Ghi DMA2_Stream2->CR = 0; Chờ bit EN về 0
    App->>DMA: 6. Nạp địa chỉ nguồn phần cứng: DMA2_Stream2->PAR = (uint32_t)&USART1->RDR
    App->>DMA: 7. Nạp địa chỉ đích RAM: DMA2_Stream2->M0AR = (uint32_t)dma_rx_buffer
    App->>DMA: 8. Nạp kích thước bộ đệm vòng: DMA2_Stream2->NDTR = BUFFER_SIZE
    App->>DMA: 9. Cấu hình CR: Channel 4 (CHSEL=4), Circular (CIRC=1), Tăng RAM (MINC=1), Ưu tiên cao (PL=10b)
    App->>DMA: 10. Kích hoạt Stream sẵn sàng đón dữ liệu: DMA2_Stream2->CR |= DMA_SxCR_EN

    Note over App,USART: BƯỚC 4: CẤU HÌNH BỘ THU/PHÁT NỐI TIẾP USART1
    App->>USART: 11. Cài đặt Baudrate 115200 bps: USART1->BRR = 938 (PCLK2 = 108MHz)
    App->>USART: 12. Bật cờ yêu cầu DMA phía bộ thu: USART1->CR3 |= USART_CR3_DMAR
    App->>USART: 13. Bật ngắt phát hiện rảnh đường truyền: USART1->CR1 |= USART_CR1_IDLEIE
    App->>USART: 14. Bật USART, Transmit & Receive: USART1->CR1 |= (USART_CR1_UE | USART_CR1_TE | USART_CR1_RE)

    Note over App,NVIC: BƯỚC 5: PHÂN MỨC ƯU TIÊN & KÍCH HOẠT NGẮT TẠI NVIC
    App->>NVIC: 15. Gán mức ưu tiên cao (Priority 2): NVIC_SetPriority(USART1_IRQn, 2)
    App->>NVIC: 16. Mở cổng ngắt: NVIC_EnableIRQ(USART1_IRQn)

    Note over App,NVIC: ➔ NGOẠI VI ĐÃ SẴN SÀNG CHẠY: Dữ liệu bay vào sẽ được DMA tự nạp vào SRAM!
```

---

### 📋 Sơ Đồ 2: Quy Trình Truyền Dữ Liệu UART Tuần Tự (UART Transmit Pipeline - Polling TXE & TC)

Mô hình hóa chu trình truyền dữ liệu từ ứng dụng, qua thanh ghi đệm `TDR`, thanh ghi dịch `Shift Register` và ra chân vật lý `PA9`:

```mermaid
sequenceDiagram
    autonumber
    actor App as Application Code (main.c)
    participant TDR as USART1->TDR (Transmit Data Reg)
    participant Shift as USART1 Shift Register
    participant ISR as USART1->ISR (Status Register)
    participant Pin as Physical Pin PA9 (TX)

    Note over App,Pin: VÒNG LẶP TRUYỀN TỪNG KÝ TỰ (CHARACTER TRANSMISSION LOOP)
    App->>ISR: 1. Kiểm tra bộ đệm truyền rảnh: while (!(USART1->ISR & USART_ISR_TXE));
    ISR-->>App: 2. Cờ TXE = 1 (TDR sẵn sàng nhận ký tự mới)
    App->>TDR: 3. Ghi ký tự: USART1->TDR = 'A';
    TDR->>ISR: 4. Phần cứng tự động xóa cờ TXE = 0
    TDR->>Shift: 5. Chuyển byte song song từ TDR sang Shift Register ➔ Phần cứng bật lại TXE = 1!
    Shift->>Pin: 6. Nối tiếp hóa (Serialization): Đưa Start Bit, 8 Data Bits, Stop Bit ra chân PA9

    Note over App,Pin: XÁC NHẬN HOÀN TẤT TOÀN BỘ CHUỖI (TRANSMISSION COMPLETE)
    App->>ISR: 7. Đợi ký tự cuối cùng bay hết ra đường truyền: while (!(USART1->ISR & USART_ISR_TC));
    ISR-->>App: 8. Cờ TC = 1 (Toàn bộ khung truyền đã ra bus an toàn, sẵn sàng ngắt nguồn hoặc chuyển mode!)
```

---

### 📋 Sơ Đồ 3: Quy Trình Vận Hành Nhận Dữ Liệu DMA Circular (Dataflow, IDLE Interrupt & D-Cache Sync)

```mermaid
sequenceDiagram
    autonumber
    participant Wire as Serial Wire (Chân PB7 RX)
    participant USART as USART1 Shift & RDR Registers
    participant DMA as DMA2 Stream 2 (Circular Mode)
    participant SRAM as Main SRAM (dma_rx_buffer)
    participant Cache as Cortex-M7 L1 D-Cache
    participant NVIC as Cortex-M7 NVIC
    participant ISR as USART1_IRQHandler()
    actor App as Application Parser

    Wire->>USART: 1. Chuỗi bit UART bay vào Shift Register ➔ Nạp byte vào USART_RDR
    USART->>DMA: 2. Phần cứng phát xung DMA Request (Channel 4)
    DMA->>SRAM: 3. DMA tự chiếm AHB bus, ghi byte vào SRAM, giảm NDTR (TẢI CPU = 0%!)
    Note over DMA,SRAM: DMA lặp lại liên tục cho mọi byte trong gói tin mà không cần CPU...

    Wire->>USART: 4. Thiết bị gửi ngừng truyền: Chân RX duy trì HIGH trong 1 Frame (10 bit times)
    USART->>USART: 5. Mạch đếm phát hiện IDLE Line ➔ Bật cờ USART_ISR_IDLE = 1
    USART->>NVIC: 6. Kích hoạt ngắt phần cứng (Do bật USART_CR1_IDLEIE)
    NVIC->>ISR: 7. CPU tạm dừng main, nhảy vào thực thi USART1_IRQHandler()

    ISR->>DMA: 8. Đọc số byte đã nhận: uint16_t count = BUFFER_SIZE - DMA2_Stream2->NDTR;
    ISR->>USART: 9. XÓA CỜ IDLE: Ghi gán USART1->ICR = USART_ICR_IDLECF (W1C, CẤM |=)
    ISR->>Cache: 10. ĐỒNG BỘ BỘ NHỚ: Gọi SCB_InvalidateDCache_by_Addr(dma_rx_buffer, BUFFER_SIZE)
    Cache->>SRAM: 11. Hủy bỏ bản photo cũ trong D-Cache, ép CPU nạp trực tiếp dữ liệu mới từ SRAM
    ISR->>App: 12. Bàn giao gói tin nguyên vẹn sang cho parser xử lý an toàn!
```

---

### 📋 Sơ Đồ 4: Quy Trình Xử Lý & Khôi Phục Khi Gặp Lỗi Phần Cứng (Hardware Error Handling: ORE/FE Recovery)

```mermaid
sequenceDiagram
    autonumber
    participant Wire as Serial Wire (Chân PB7 RX)
    participant USART as USART1 Hardware Engine
    participant DMA as DMA2 Stream 2 Controller
    participant ISR as USART1_IRQHandler()
    actor App as Firmware Supervisor

    Wire->>USART: 1. Byte mới bay đến chân RX khi thanh ghi RDR vẫn còn dữ liệu chưa đọc
    USART->>USART: 2. Phần cứng phát hiện quá tải: Bật cờ lỗi Overrun ORE = 1 trong USART_ISR
    USART->>DMA: 3. KHÓA PHẦN CỨNG: USART lập tức VÔ HIỆU HÓA xung DMA Request để bảo vệ bộ nhớ!
    Note over USART,DMA: ⚠️ NGUY HIỂM: Nếu không xóa cờ ORE, DMA sẽ ĐỨNG HÌNH VĨNH VIỄN!

    USART->>ISR: 4. Bắn ngắt USART1_IRQHandler (Do bật ngắt hoặc cờ ORE kích hoạt)
    ISR->>USART: 5. Đọc cờ lỗi: if (USART1->ISR & USART_ISR_ORE)
    ISR->>USART: 6. GIẢI TỎA LỖI (Clear Flag): Ghi gán USART1->ICR = USART_ICR_ORECF
    USART->>USART: 7. Phần cứng xóa cờ ORE về 0
    USART->>DMA: 8. Tái kích hoạt luồng phát DMA Request bình thường trở lại
    ISR->>App: 9. Tăng bộ đếm thống kê lỗi rx_overrun_count để giám sát chất lượng đường truyền
```
...
/**
 * @brief Lấy vị trí con trỏ Head (vị trí DMA vừa cất byte mới nhất vào RAM)
 * @return Index từ 0 đến (UART_RX_BUFFER_SIZE - 1)
 */
uint16_t UART1_DMA_GetReadIndex(void) {
...
int main(void) {
    ...
    uint16_t old_pos = 0; // Con trỏ Tail (vị trí đã đọc lần trước)

    while (1) {
        if (uart_rx_idle_flag) {
            uart_rx_idle_flag = 0; // Xóa cờ mềm
            
            // 1. Lấy vị trí Head mà DMA vừa ghi tới trong SRAM
            uint16_t current_pos = UART1_DMA_GetReadIndex();
            
            // 2. Đọc toàn bộ các byte mới từ old_pos đến current_pos
            for (uint16_t i = old_pos; i != current_pos; i = (i + 1) % UART_RX_BUFFER_SIZE) {
                uint8_t byte_nhan = dma_rx_ring_buffer[i];
                // Xử lý gói tin byte_nhan tại đây...
            }
            
            // 3. Cập nhật vị trí cũ
            old_pos = current_pos;
        }
    }
}
...
👉 **Bước tiếp theo:** Chuyển sang [**`📗 [NGÀY 3] Hướng dẫn Bare-metal: bxCAN Controller & Filter Bank Configuration`**](file:///d:/Project/STM32F7/day03_learning_guide.md)!
